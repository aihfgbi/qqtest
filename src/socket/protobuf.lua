...
local function default_table(typename)
	...
	local default_inst = decode_message(typename , "")
    if default_inst == nil then
        print("proto warning!!!===>",typename,"is nil")
        return nil
    end
	v = { 
...
function decode(typename, buffer, length)
	...
	if ok then
        local tab = setmetatable(ret , default_table(typename))
        local function _decode(value)
        	if type(value) == "table" then
        	   for k,v in pairs(value) do
        	       if type(v)=="string" and string.find(v,"ProtoNet") then
                        expand(value)
                        break
        	       end
        	   end
		...
		return false , print("decode error==>",c._last_error(P))
	end
end
